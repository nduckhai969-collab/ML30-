...
class MLTrainer:
    ...
    def _build_classifier(
        ...
        if name == "svm":
            ...
            raise ValueError(
                f"classifier_name: 'svm', 'knn', hoặc 'random_forest'. "
                f"Nhận được: '{name}'")
    ...
    def fit(
        ...
        X_train, y_train = self.extract_features(train_loader,desc="Train")
        print("Extracting val features...")
        X_val, y_val = self.extract_features(val_loader,desc="Val")
        self.classifier.fit(X_train, y_train)
        val_preds = self.classifier.predict(X_val)
        val_acc = accuracy_score(y_val, val_preds) * 100
        self.cpu_ram_mb = (psutil.Process().memory_info().rss / 1024 / 1024)
        print(f"\nTraining Finished")
    ...
    def evaluate_test(
        ...
        self._save_confusion_matrix(y_test, preds, class_names)
    ...
    def _save_confusion_matrix(
        ...
        disp.plot(
            ...
            xticks_rotation=45,
            colorbar=False)
        plt.title(f"Confusion Matrix — {self.classifier_name.upper()}" )
        plt.tight_layout()
