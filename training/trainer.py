...
# FIX: dùng torch.amp thay vì torch.cuda.amp (deprecated từ PyTorch 2.0)
...
class Trainer:
    ...
    def __init__(
        ...
        # FIX: GradScaler mới yêu cầu truyền device_type
        self.scaler = GradScaler(
            device="cuda",
            enabled=torch.cuda.is_available()
        )

        # Best model theo val_acc — dùng để early stopping và load lại cuối training
        self.best_acc = 0.0
        self.counter = 0

        # Best loss — chỉ lưu file riêng, không dùng để early stopping
        ...
        self.inference_time = 0      # forward pass thuần (ms/ảnh)
        self.end_to_end_time = 0     # preprocessing + forward pass (ms/ảnh)
        self.model_params = 0
    ...
    def train_one_epoch(self):
        ...
        for images, labels in loader:
            ...
            # FIX: autocast mới yêu cầu device_type
            with autocast(
                device_type="cuda",
                enabled=torch.cuda.is_available()
            ):
    ...
    def validate(self):
        ...
        # Lưu best_loss model riêng — không liên quan đến early stopping
    ...
    def measure_inference_time(
        ...
        """
        Đo 2 loại thời gian (ms/ảnh):

        - inference_time   : forward pass thuần
                             (tensor đã lên GPU → logits ra)
        - end_to_end_time  : preprocessing + forward pass
                             (fetch batch từ DataLoader → logits ra)
        """
        ...
        with torch.no_grad():
            ...
            for idx, (images, _) in enumerate(loader):
                ...
                # ------------------------------------------
                # End-to-end: tính từ lúc bắt đầu fetch batch
                # (images đã được DataLoader trả về ở trên)
                # ------------------------------------------
                if torch.cuda.is_available():
                ...
                # ------------------------------------------
                # Inference thuần: chỉ tính forward pass
                # ------------------------------------------
                if torch.cuda.is_available():
                    torch.cuda.synchronize()
                ...
                e2e_end = infer_end  # kết thúc e2e = kết thúc forward
    ...
    def save_curves(self):
        ...
        plt.savefig(
            os.path.join(self.save_dir, "loss_curve.png")
        )
        plt.close()
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            # Kích hoạt fine-tuning tại epoch chỉ định
            if (
        ...
        # Load lại best_acc_model để evaluate_test dùng đúng model tốt nhất
        ...
        self.model.load_state_dict(
            torch.load(
                ...
                weights_only=True  # FIX: tránh warning từ PyTorch 2.0+
            )
        )
